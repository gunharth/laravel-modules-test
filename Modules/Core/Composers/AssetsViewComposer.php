...
class AssetsViewComposer
{
    ...
    public function compose(View $view)
    {
        ...
        foreach (config('sorter.core.core.admin-assets') as $assetName => $path) {
            $path = $this->assetFactory->make($path)->url();
            $this->assetManager->addAsset($assetName, $path);
        }
        $this->assetPipeline->requireCss(config('sorter.core.core.admin-required-assets.css'));
        $this->assetPipeline->requireJs(config('sorter.core.core.admin-required-assets.js'));
